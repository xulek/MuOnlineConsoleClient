...
namespace MuOnlineConsole.Networking.PacketHandling.Handlers
{
    ...
    public class InventoryHandler : IGamePacketHandler
    {
        ...
        private readonly SimpleLoginClient _client; // Needed for pickup flags and movement handling
        private readonly TargetProtocolVersion _targetVersion;

        public InventoryHandler(ILoggerFactory loggerFactory, CharacterState characterState, SimpleLoginClient client, TargetProtocolVersion targetVersion)
        {
        ...
        public Task HandleItemAddedToInventoryAsync(Memory<byte> packet)
        {
            ...
            try
            {
                ...
                if (packet.Length < minLength + 1) { _logger.LogWarning("⚠️ Received ItemAddedToInventory packet (0x22) too short. Length: {Length}. Packet: {PacketData}", packet.Length, Convert.ToHexString(packet.Span)); _client.LastPickupSucceeded = false; _client.PickupHandled = true; _client.SignalMovementHandledIfWalking(); return Task.CompletedTask; }

                var itemAdded = new ItemAddedToInventory(packet);
                var itemData = itemAdded.ItemData;
                _client.PickupHandled = true;
                if (itemData.Length < 1) { _logger.LogWarning("⚠️ ItemData in ItemAddedToInventory is empty. Raw packet: {PacketData}", Convert.ToHexString(packet.Span)); _client.LastPickupSucceeded = false; _client.SignalMovementHandledIfWalking(); return Task.CompletedTask; }

                _client.LastPickupSucceeded = true;
