...
using MuOnlineConsole.Networking.Services; // For CharacterService
...
namespace MuOnlineConsole.Networking.PacketHandling.Handlers
{
    ...
    public class MiscGamePacketHandler : IGamePacketHandler
    {
        ...
        private readonly SimpleLoginClient _client; // Needed for state transitions etc.
        ...
        public MiscGamePacketHandler(ILoggerFactory loggerFactory, SimpleLoginClient client, CharacterService characterService, TargetProtocolVersion targetVersion)
        {
        ...
        public Task HandleCharacterListAsync(Memory<byte> packet)
        {
            try
            {
                ...
                if (characters.Count > 0) { Task.Run(() => _client.SelectCharacterInteractivelyAsync(characters)); }
