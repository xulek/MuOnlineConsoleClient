...
using MuOnlineConsole.Client; // For SimpleLoginClient, CharacterState, ScopeManager, TargetProtocolVersion
using MuOnlineConsole.Configuration; // For MuOnlineSettings
using MuOnlineConsole.Core.Models; // For ServerInfo
using MuOnlineConsole.Core.Utilities; // For PacketHandlerAttribute, SubCodeHolder, ItemDatabase
using MuOnlineConsole.Networking.Services; // For CharacterService, LoginService
using MuOnlineConsole.Networking.PacketHandling.Handlers; // For the handler classes

namespace MuOnlineConsole.Networking.PacketHandling
{
    /// <summary>
    ...
    public class PacketRouter
    {
        ...
        private readonly SimpleLoginClient _client; // Client instance for state management and UI interactions
            ...
            MuOnlineSettings settings)
        {
            ...
            _characterDataHandler = new CharacterDataHandler(loggerFactory, characterState, client, targetVersion);
            _inventoryHandler = new InventoryHandler(loggerFactory, characterState, client, targetVersion);
            _scopeHandler = new ScopeHandler(loggerFactory, scopeManager, characterState, client, targetVersion);
            _chatMessageHandler = new ChatMessageHandler(loggerFactory);
            _connectServerHandler = new ConnectServerHandler(loggerFactory, client);
            _miscGamePacketHandler = new MiscGamePacketHandler(loggerFactory, client, characterService, targetVersion);
        ...
        private async Task ExecuteHandler(Func<Memory<byte>, Task> handler, Memory<byte> packet, byte code, byte subCode)
        {
            ...
            catch (Exception ex)
            {
                ...
                {
                    _client.SignalMovementHandledIfWalking(); // Signal movement handling to release walk lock
                }
            }
        }
